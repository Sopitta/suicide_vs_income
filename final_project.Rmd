...
The kind of priors chosen by us for sensitivity analysis are all different kinds of proper, uninformative priors. That is because, this being a regression problem, there weren't any informative priors we could readily use. The use of the hierarchical model allows the models to developed a shared common prior, but it remains to be seen how useful that would be. The final chosen priors are given below the sensitivity analysis, with the corresponding result. 
...
```
The tree-depth never exceeds 6 and hence seems to be comfortably low. 
Divergences: 
...
While the minimum N_eff is 1700, the mean is about 3800 indicating a good fit and succesful MCSM. 
