...
The kind of priors chosen by us for are all different kinds of proper, uninformative priors. That is because, this being a regression problem, there weren't any informative priors we could readily use. We typically use large variance normal distributions for the mean priors and hyperpriors, and large variance gamma distributions for the variance priors and hyperpriors. The use of the hierarchical model allows the models to developed a shared common prior, but it remains to be seen how useful that would be. **The final chosen priors are given below the sensitivity analysis, with the corresponding result.** 
...
It appears that the performance of all 4 models are very much comparable. The differing trends between the three contexts might be what lead the hierarchical models to perform rather poorly - the separated models emerges as the best option. The worst option with the lowest ELPD is the pooled model, which makes sense considering how different our three contexts are. Within the hierarchical models, the model with correlated regression parameter appears to do marginally better at fitting our data, judging from the ELPD values. 
The ELPD LOO values gives the expected log predictive values when leaving out each prediction one at a turn. It gives a good measure of likelihood and therefore is a reasonable metric to measure how well the model fits our data. 

## Plotting results for the best model 

The ELPD values obtained are >-400, being not very high and indicating an imperfect fit of the data. We are unsure of what kind of values would indicate a good fit, but we just chose the ones that the best amongst what we had. 
The best model, accordings to these results is Model 3, with separate models. This could be because the trends in all three contexts are quite different, and hence there is little benefit derived in trying to analyse them together.
...
lines(ukdata$wagepercol, mu1v[,2], col='blue', lty=2)
lines(ukdata$wagepercol, mu1v[,3], col='blue', lty=2)
```

There is a strong link between poverty and the suicide rate. In the figure above, the bold line indicates the mean predicted suicide rate for each income value, while the dotted lines indicate the 95% confidence interval of that mean. 

```{r} 
#The second column of t1 is the stimated Beta or slope parameter
quantile(list_of_draws$t1[,2], c(0.025, 0.5, 0.975))
```
...
lines(usdata$wagepercol, mu2v[,2], col='blue', lty=3)
lines(usdata$wagepercol, mu2v[,3], col='blue', lty=3)


```

There is a weaker link between poverty and the suicide rate. In the figure above, the bold line indicates the mean predicted suicide rate for each income value, while the dotted lines indicate the 95% confidence interval of that mean. 

```{r} 
#The second column of t1 is the stimated Beta or slope parameter
quantile(list_of_draws$t2[,2], c(0.025, 0.5, 0.975))
```
The values of slope appear less negative as compared to above, the mean of the slope is in fact close to zero, indicating the suicide rate is largely independent of income level. 
...
lines(countrydata$wagepercol, mu3v[,2], col='blue', lty=3)
lines(countrydata$wagepercol, mu3v[,3], col='blue', lty=3)
...
```{r} 
#The second column of t1 is the stimated Beta or slope parameter
quantile(list_of_draws$t3[,2], c(0.025, 0.5, 0.975))
```
It can see that for the entire 95% interval, the slope for this context is positive. 
...
print(c('Min tree depth: ', min_tree_depth, 'Average tree depth: ',  mean_tree_depth, 'Max tree depth: ', max_tree_depth))
```
The tree-depth never exceeds 6 and hence seems to be comfortably low. In general lesser tree depth means a better model, it is less complex and would take less time to fit. 
**Divergences:**
...
print(c('Minimum Divergences: ', min_div,'Average Divergences: ',  mean_div, 'Maximum Divergences: ',  max_div))
```

There are no divergences in the draws. Divergences indicate when the trajectory of a modelled parameter differs from the true trajectory, and usually indicates poor fit. In this case, the fit appears healthy. 
...
While the minimum N_eff is 1700, the mean is about 3800 indicating a good fit and succesful MCSM. The sampling appears to have occured succesfully.
From the given diagnostics, it is clear that the model has converged nicely! 
...
Since the ELPD values weren't very high, we can maybe conclude that the models we have designed aren't doing a great job with the data. It is difficult to estimate as we don't know what a reasonable value would be. 
**Potential improvements:** It seems like the global context data has very different trends as compared to the UK and US context data, even though the data was standardized, and its inclusion with the other two might have compromised performance, which is why the separate model appeared to have performed better. A potential improvement for modelling might have the global data excluded and more country specific, profession-wise data included. 


### Reflections 

This is the first bayesian data analysis project for both of us where we modelled real-world data that we assembled ourselves. We have gained better undertstanding of each model type, Stan functions, model comparison, and most importantly, learned to construct a data analysis project from the start as a team. An interesting lesson we learned is that complicated models may not necessarily be the best one for a given application. In this project, the simple separate model triumphed over the complicated hierarchial models with correlated hyper-priors (someething that required a lot of research for us to figure out how to implement.) Also it was the first time we had looked under the hood at many diagnostics, and that was interesting to us as well. 
